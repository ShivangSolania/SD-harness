...
4. **Output cap:** Limit each section (Affected Files, Import Boundaries, Risk Surface) to the 20 highest-signal entries. If more exist, append a `> N more items omitted — query architecture.md directly` note. This prevents premature context-window saturation.
5. After completing a research scan, write a 1-3 line summary of your findings to `context/summaries/<feature-or-file-name>.md` for future session reuse. If a summary already exists for that target, update it in-place rather than appending.
